...
def demo_with_llm_data(sample_llm_output: Union[str, Dict, List]) -> None:
    """
    Demonstrate the classifier with LLM-generated nutrition data
    
    Args:
        sample_llm_output: Sample nutrition data from an LLM
    """
    # Create the categorizer
    categorizer = EnhancedFoodCategorizer(multi_label=True)
    
    # Process LLM data
    food_data = categorizer.process_llm_nutrition_data(sample_llm_output)
    ...
    if len(food_data) >= 10:
        ...
        # With limited data, just train on everything
        categorizer.train(food_data)
    
    # Make predictions on some sample items
    sample_foods = [
    ...
    predictions = categorizer.predict(sample_foods)
    
    print("\nSample Predictions:")
    for food, categories in zip(sample_foods, predictions):
...
# Example of how to integrate with the LLM output
def integrate_with_llm_api(food_items: List[str], llm_api_function) -> pd.DataFrame:
    """
    Get nutritional information from LLM API and prepare for classification
    
    Args:
        food_items: List of food items to analyze
        llm_api_function: Function to call the LLM API
        
    Returns:
        pd.DataFrame: Food data with nutritional information
    """
    all_food_data = []
    ...
    for item in food_items:
        ...
        prompt = f"""
        Analyze the following food item and provide its nutritional information:
        {item}
        ...
        # Call the LLM API
        llm_response = llm_api_function(prompt)
        
        try:
            # Parse the response as JSON
            food_data = json.loads(llm_response)
            all_food_data.append(food_data)
        except json.JSONDecodeError:
            logger.error(f"Failed to parse LLM response as JSON: {llm_response[:100]}...")
    
    # Convert to DataFrame
    if all_food_data:
        return pd.DataFrame(all_food_data)
    else:
        return pd.DataFrame()